...
from .serializers import RegisterSerializer, SetNewPasswordSerializer, ResetPasswordEmailRequestSerializer, \
	ActivationCodeSerializer, LoginSerializer, LogoutSerializer
...
class RegisterView(generics.GenericAPIView):
	...
	def post(self, request):
		...
		activation_code = ActivationCode.objects.get(user=user, created_date__lte=(dt.now() + timedelta(days=1)))
		current_site = get_current_site(request).domain
		relativeLink = reverse('email-verify')
		absurl = 'http://' + current_site + relativeLink

		email_body = f'Hi {user.username} Use the passcode below to verify your email {activation_code.passcode}\n {absurl}'
...
class ColumnListCreateView(APIView):
	...
	def get(self, request, desk_pk):
		column_obj = Column.objects.filter(
			(Q(desk__author=request.user) | Q(desk__members__user=request.user)) & (Q(desk_id=desk_pk)))
...
class CardDetail(APIView):
	...
	permission_classes = (IsDeskOwnerOrMember, IsAuthenticated)
	parser_classes = [MultiPartParser, FormParser]
...
class CardListCreateView(APIView):
	...
	def get(self, request, column_pk, *args, **kwargs):
		card_obj = Card.objects.filter(
			(Q(column__desk__author=request.user) | Q(column__desk__members__user=request.user)) & (
				Q(column_id=column_pk)))
...
class AcceptInvite(APIView):
	...
	def post(self, request, desk_id):
		user = request.user
		desk = MainDesk.objects.filter(id=desk_id).first()

		if desk is None:
			return Response({"error": "does not exist desk"}, status.HTTP_400_BAD_REQUEST)

		MemberDesk.objects.create(user=user, desk_id=desk_id)

		return Response({'status': 'good'}, status.HTTP_200_OK)
